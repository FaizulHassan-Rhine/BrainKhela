import { diffIndex, pickItems, orderOptions } from '@/lib/brain-games/helpers';

const ROUNDS = { easy: 5, medium: 8, hard: 10 };

export function getGamePlayConfig(game, lang = 'bn', difficulty = 'medium', preview = false) {
  const d = game.dataset || {};
  const di = diffIndex(difficulty);
  const rounds = ROUNDS[difficulty];
  const pick = (arr, count) => pickItems(arr, count, preview);

  switch (game.id) {
    case 'g01': {
      const grid = d.grid_sizes?.[di] || 12;
      const pairs = Math.floor(grid / 2);
      const icons = pick(d.icons || [], pairs);
      return {
        engine: 'memory-pairs',
      ...
      return {
        ...
        props: {
          items: (d.sample || []).map((s) => {
            const built = buildOptions(s.answer, preview);
            return {
      ...
      return {
        ...
        props: {
          items: (d.sample || []).map((s) => {
            ...
            const opts = orderOptions([s.area, s.perimeter, s.area + 1, s.perimeter + 1].map(String), preview);
      ...
      return {
        ...
        props: {
          items: (d.pairs || []).map((p) => {
            ...
            const wrong = pick((lang === 'en' ? p.wrong_en : p.wrong_bn) || [], 3);
            const options = orderOptions([answer, ...wrong].filter(Boolean), preview);
      ...
      return {
        ...
        props: {
          items: (d.items || []).map((item) => {
            ...
            const wrong = pick(d.items.filter((x) => x !== item), 3).map((x) => (lang === 'en' ? x.meaning_en : x.meaning_bn));
            const options = orderOptions([meaning, ...wrong], preview);
      ...
      return {
        ...
        props: {
          items: (d.items || []).map((item) => {
            ...
            const options = orderOptions([correct, ...(wrong || [])], preview);
            return {
      ...
      return {
        ...
        props: {
          items: (d.items || []).map((item) => {
            ...
            const wrong = pick((d.items || []).filter((x) => x !== item), 3).map((x) => (lang === 'en' ? x.answer_en : x.answer_bn));
            const options = orderOptions([answer, ...wrong], preview);
            return {
...
function buildOptions(answer, preview = false) {
  const n = Number(answer);
  const wrong = [n + 1, n - 1, n + 5].filter((x) => x !== n && x >= 0);
  const options = orderOptions([String(answer), ...wrong.map(String)], preview);
  return { options, correct: options.indexOf(String(answer)) };
}

export function getGamePreviewConfig(game, lang = 'bn') {
  return getGamePlayConfig(game, lang, 'easy', true);
}
